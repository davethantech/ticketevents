...
create table if not exists public.scans (
  ...
  code text not null, result text not null, by_user uuid references auth.users(id) on delete set null, created_at timestamptz not null default now()
);
...
-- Drop the broad first-pass policies so scanner accounts cannot mutate event configuration or financial data.
do $$ declare p record; begin
  for p in select policyname, tablename from pg_policies where schemaname='public' and policyname in (
    'members can read organizations','members can read memberships','admins manage memberships','members read events','staff manage events',
    'members manage tickets','members manage guests','members manage expenses','members manage tables','members read orders','members manage scans','members manage messages'
...
-- Public guest experiences should use a narrowly scoped Edge Function/RPC for invite lookup and RSVP updates.
-- Do not expose the full guests table anonymously.
