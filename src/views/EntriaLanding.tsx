...
export function StaffLogin({ onSuccess, onBack }: { onSuccess: () => void; onBack: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    // Presentation-build gate. Replace with server/Supabase authentication for production.
      ...
      <div className="relative mx-auto flex min-h-screen max-w-[1280px] items-center justify-center px-5 py-10">
        <div className="grid w-full max-w-5xl overflow-hidden rounded-[34px] border border-white/10 bg-white/[.045] shadow-[0_50px_140px_rgba(0,0,0,.55)] backdrop-blur-xl lg:grid-cols-[1.05fr_.95fr]">
          <div className="relative hidden min-h-[680px] overflow-hidden border-r border-white/10 p-10 lg:block">
            <div className="absolute -left-24 top-20 h-80 w-80 rounded-full bg-emerald-400/10 blur-[90px]" />
            <div className="absolute -right-20 bottom-10 h-96 w-96 rounded-full bg-amber-300/10 blur-[100px]" />
            <div className="relative flex h-full flex-col justify-between">
              ...
              <div>
                <div className="mb-5 text-[9px] font-bold uppercase tracking-[.28em] text-amber-200/60">Private staff portal</div>
                <h1 className="font-display text-6xl font-black leading-[.88] tracking-[-.05em]">CONTROL<br/><span className="text-amber-200">THE GATE.</span></h1>
                <p className="mt-6 max-w-md text-sm leading-7 text-white/45">Create events, manage guest lists, send invitations, monitor capacity and operate QR check-in from one protected workspace.</p>
                <div className="mt-10 grid grid-cols-2 gap-3">{["Event creation", "Guest intelligence", "QR check-in", "Live reporting"].map((x) => <div key={x} className="rounded-2xl border border-white/10 bg-white/[.035] px-4 py-4 text-[10px] font-bold uppercase tracking-[.16em] text-white/55">{x}</div>)}</div>
              </div>
          ...
          <div className="flex min-h-[680px] flex-col justify-center p-7 sm:p-10">
            ...
            <form onSubmit={submit} className="space-y-4">
              ...
              <button type="submit" className="h-12 w-full rounded-xl bg-white text-[11px] font-black uppercase tracking-[.18em] text-black transition hover:-translate-y-0.5">Sign in</button>
            </form>
            <div className="my-6 flex items-center gap-3"><span className="h-px flex-1 bg-white/10"/><span className="text-[9px] uppercase tracking-[.2em] text-white/20">presentation build</span><span className="h-px flex-1 bg-white/10"/></div>
            <button onClick={demo} className="h-11 w-full rounded-xl border border-amber-200/20 bg-amber-100/[.05] text-[10px] font-bold uppercase tracking-[.16em] text-amber-100/80 transition hover:bg-amber-100/10">{preview ? "Opening staff workspace…" : "Use demo staff access"}</button>
            <p className="mt-5 text-center text-[9px] leading-5 text-white/25">Preview gate only. Production must enforce authentication and role-based access on the server, not in the browser.</p>
          </div>
        </div>
      </div>
    </div>
