...
const go = (path: string) => {
  if (path.startsWith("#")) { document.querySelector(path)?.scrollIntoView({ behavior: "smooth" }); return; }
  window.location.hash = path; window.scrollTo({ top: 0, behavior: "smooth" });
};
  ...
  return <div className="entria-public">
    ...
    <section id="events" className="ep-section ep-cream">
      ...
      <div className="ep-events">
        <article className="ep-event"><img src={events[0].image} alt=""/><div className="ep-event-shade"/><div className="ep-event-info"><span className="ep-event-tag">{events[0].type}</span><h3>{events[0].name}</h3><p>{events[0].date} · {events[0].time}<br/>📍 {events[0].venue}, {events[0].city}</p><div className="ep-event-price">From <strong>{events[0].price}</strong></div></div></article>
        <div className="grid gap-3">{events.slice(1).map((e) => <article key={e.name} className="ep-event small"><img src={e.image} alt=""/><div className="ep-event-shade"/><div className="ep-event-info"><span className="ep-event-tag">{e.type}</span><h3>{e.name}</h3><p>{e.date} · {e.time}<br/>📍 {e.venue}, {e.city}</p><div className="ep-event-price">From <strong>{e.price}</strong></div></div></article>)}</div>
      </div>
    </section>
    <section id="guest-experience" className="ep-section ep-black">
      ...
      <div className="ep-cards">{[
        ["01","Personalised digital invites","Every guest can receive a branded invitation, private access code and event-specific RSVP experience."],
        ["02","RSVP & guest control","Know who is invited, who confirmed, how many guests each invitation permits and exactly who is expected."],
        ["03","WhatsApp + email communication","Automated invitations, reminders, confirmations and post-event thank-you messages keep organisers hands-free."],
        ["04","Guest POV gallery","A private event gallery lets guests share photos, videos and voice notes from their own phones without creating a public name list."],
      ].map(([n,title,copy]) => <article className="ep-card" key={n}><span className="ep-num">{n}</span><h3>{title}</h3><p>{copy}</p></article>)}</div>
    </section>
    <section id="security" className="ep-section ep-cream">
      <div className="ep-showcase">
        ...
        <div className="ep-showcase-copy"><span className="ep-label">CONTROLLED ACCESS</span><h3>Keep the guest list at the door.</h3><p>The core business problem is simple: if one invitation can silently become five people at the gate, the organiser loses capacity control and money. Entria ties each pass to an exact permitted headcount and records entry once.</p><div className="ep-list">
          <div><b>01</b><span><strong>ONE-TIME QR</strong> A used credential immediately becomes invalid for another entry.</span></div>
          <div><b>02</b><span><strong>HEADCOUNT ENFORCEMENT</strong> A pass for two is admission for two — not an invitation for an unlimited group.</span></div>
          <div><b>03</b><span><strong>OFFLINE GATEKEEPER</strong> The scanner can keep an event manifest locally and queue scans when connectivity disappears.</span></div>
          <div><b>04</b><span><strong>PRIVACY FIRST</strong> Gate staff see only the minimum information needed to make a safe admission decision.</span></div>
        </div></div>
      </div>
    </section>
